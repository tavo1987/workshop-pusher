...
<script>
    ...
    function addActivity(type, data) {
        ...
        if(type == 'status-update-liked'){
            var activityLiked = $('#activities').find('[data-activity-id="'+data.liked_activity_id+'"]');
            var $likeCount = activityLiked.find('.like-count');
            var $count =   1;

            if($likeCount.text().length > 0){
    ...
    //Handle the liked event
    function addLikedUpdate(data) {
